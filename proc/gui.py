...
class Main(QMainWindow):
	...
	def Add_Row(self):
		...
		if ip is None or port is None:
			self.writeTerminal("[!] Target is not desginated.\n")
			return
		...
		self.nmTable.setItem(rowPosition, 3, QTableWidgetItem("disconnected"))
		self.nmTable.setItem(rowPosition, 4, QTableWidgetItem("undetectable"))
		self.nmTable.resizeRowsToContents()
		self.count+=1
		self.statusBar().showMessage('Register target machine')
		self.writeTerminal("[+] ip_{0} Machine was registered at {1}.\n".format(ip,time.ctime()))
	def Delete_Row(self):
		...
		try:
			...
			self.writeTerminal("[-] ip_{0} Machine was removed at {1}\n".format(ip,time.ctime()))
	...
	def Disconnect_Row(self):
		...
		try:
			...
			self.writeTerminal("[-] ip_{0} Machine was disconnected at {1}.\n".format(ip,time.ctime()))
			self.host_list.pop(ip)
			self.nmTable.setItem(rowPosition, 3, QTableWidgetItem("dead"))
			return
		except KeyError:
			self.writeTerminal("[!] Already disconnected. Key dumped.\n")
			self.statusBar().showMessage('Already disconnected.')
		except:
			self.writeTerminal("[!] Object lost when removing target machine.\n")
			self.statusBar().showMessage('Error : when try to delete target machine in list.')
	...
	def Connect_Target(self):
		...
		if ip is None or port is None:
			self.writeTerminal("[!] connection error : target lost.\n")
		...
		res = self.login(passwd,ip,port)
		if res==False:
			self.writeTerminal("[!] {0} Machine Log in Failed at {1}.\n".format(ip,time.ctime()))
			self.statusBar().showMessage('Error : failed to connect')
			return
		self.writeTerminal("[+] ip_{0} Machine is connected at {1}\n".format(ip,time.ctime()))
		self.statusBar().showMessage('{0} conenct success with target machine.'.format(ip))
		self.nmTable.setItem(row, 3, QTableWidgetItem("idle"))
	...
	def Run_Script(self):
		...
		if ip is None or run is None:
			self.writeTerminal("[!] run script : target lost.\n")
			self.statusBar().showMessage('Error : Need selected target.')
			return
		ip 		= ip.text()
		run 	= run.text()
		...
		if ret:
			self.writeTerminal("[*] ip_{0} Machine is running script at {1}.\n".format(ip,time.ctime()))
	...
	def Stop_Script(self):
		...
		ip 		= self.nmTable.item(row, 1)
		if ip is None:
			self.writeTerminal("[!] stop script : target lost.\n")
			self.statusBar().showMessage('Error : Need selected target')
			return
		ip		= ip.text()
		self.writeTerminal("[*] ip_{0} Machine stoped script at {1}.\n".format(ip,time.ctime()))
	...
	def communicate(self,ip,sets):
		try:
			...
			self.statusBar().showMessage('command failure. Target host missing'.format(ip))
			self.msessageBox(QMessageBox.Information,"Info","Host already down")
			return 0
...
class Terminal(QtCore.QThread):
	...
	def transporter(self,Name):
		...
		cipher_suite = Fernet(key)
		self.writeTerminal.emit('[*] Generate file transfer thread...\n')
	...
	def run(self):
		...
		if self.command[0] == 'scan#1':
			...
			Name = self.command[1]
			self.writeTerminal.emit('[*] compiler : src with {0}\n'.format(Name))
			self.writeTerminal.emit('[*] compiler : Start compile...\n')
			Compiler = Preprocessor()
			ret 	 = Compiler.compile(Name)
			if ret is True:ret = "Success"
			else:ret = " Failed.\n[-] compile error at line_No {0}".format(ret)
			self.writeTerminal.emit('[*] compiler : Compiled Result : {0}\n'.format(ret))
